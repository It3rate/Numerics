...
public class PRangeConversionTests
{
    ...
    {
        var n1 = new Number(_domain, new(2000, 3000), Polarity.Aligned); // (-2i+3)
        var n2 = new Number(_domain, new(0, -1000), Polarity.Inverted); // ~(1)
        var pr1 = PRange.FromNumber(n1);
        var pr2 = PRange.FromNumber(n2);
        var result = n1 * n2;
    ...
    public void Division_invTests()
    {
        ...
        _domain = new Domain(_trait, _basisFocal, _limits);
        var n = new Number(_domain, new(40000, 50000), Polarity.Aligned); // (-4i+5) => ~(4-5i)
        var prn = PRange.FromNumber(n);

        var denom = new Number(_domain, new(-20000, 30000), Polarity.Inverted); // ~(-2-3i) => (2i+3)
